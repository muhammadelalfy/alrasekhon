...
trait FileUploadTrait
{
                           ...
                           array $data = []): bool|File
    {
        ...
        if (empty($folder)) {
            ...
            $folder = config("dispatcher.upload.paths.{$folder}", config('dispatcher.upload.paths.default'));
        }
        ...
        if (Str::contains($data['mime'], 'image')) {
            ...
            Storage::put($data['url'], $image->stream($data['ext'], config('dispatcher.upload.quality')));
        } else {
    ...
    {
        $maxWidth = config('dispatcher.upload.max_width');
